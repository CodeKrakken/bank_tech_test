...
describe 'account' do
  ...
  it 'allows the account holder to credit 10' do
    expect(@account.credit(10)).to eq("#{@today} || 10 || || 10")
  end

  it 'keeps a running balance' do
    @account.credit(10)
    expect(@account.credit(10)).to eq("#{@today} || 10 || || 20")
  end

  it 'allows the account holder to debit 10' do
    expect(@account.debit(10)).to eq("#{@today} || || 10 || -10")
  end
  ...
  it 'keeps track of a combination of debits and credits' do
    ...
    @account.credit(10)
    @account.debit(5)
    expect(@account.debit(35)).to eq("#{@today} || || 35 || -20")
  end
